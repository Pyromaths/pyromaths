...
from outils.Affichage import TeX
import random
...
def plus_dec(pyromax):
    (a, b) = (outils.Arithmetique.valeur_alea(-10*pyromax, 10*pyromax)/10.0, outils.Arithmetique.valeur_alea(-10*pyromax, 10*pyromax)/10.0)
    return (a, b)
...
def main():
    ...
    for j in range(14,20):
        ...
        if calculs[j] // 10 == 0:
            choix_trou(TeX(a), outils.Affichage.tex_coef(b, '', bpn=1), TeX(a + b), '+', exo,
                       cor)
        if calculs[j] // 10 == 1:
            choix_trou(TeX(a), outils.Affichage.tex_coef(b, '', bpn=1), TeX(a - b), '-', exo,
                       cor)
