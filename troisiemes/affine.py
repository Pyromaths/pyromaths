...
def anteimage(fonc,A,B):
    ...
    l=['l\'image de ','un nombre qui a pour image ','un antécédent de ']
    lcor=['l\'image de ',' a pour image ','un antécédent de '] #liste pour le corrigé
...
#Génère la 2e queston et sa réponse
    ...
    l=['Tracer la droite représentative ($d_'+str(i)+'$) de la fonction $'+f+':x\\longmapsto '+coef+'x'+b+'$.',
       st,
...
#A est sur l'axe des ordonnées, f est le nom de la fonction
    ...
    l=['Déterminer l\'expression de la fonction $'+f+'$ représentée ci-contre par la droite ($d_'+str(i)+'$).',
       'On lit l\'ordonné à l\'origine et le coefficient de la fonction affine sur le graphique.\\\ ',
...
def affine(f0,f1):
    ...
    exo=["\\parbox{0.5\\linewidth}{",
         "($d_1$) est la droite représentative de la fonction $"+fonc1+"$.",
         "\\begin{enumerate}",
         "\\item "+l[0],
         "\\item "+l[4],
         "\\item "+l2[0],
