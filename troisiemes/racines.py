...
def valeurs_aRb0(pyromax):  # renvoie (coef0, coef1, coef2, carre0, carre1, carre2, b)
    a = carres
    l = [outils.Arithmetique.valeur_alea(-pyromax, pyromax) for i in range(3)]
    while True:
...
def valeurs_aPbRc(pyromax):  # renvoie (coef0, coef1, coef2, carre0, carre1, carre2, b)
    while True:
        ...
        if a != b and outils.Arithmetique.pgcd(a, b) != min(a, b):
            break
    while True:
        c = random.randrange(2, pyromax)
        d = outils.Arithmetique.valeur_alea(-pyromax, pyromax)
        if c != d:
            break
    return (c, a, d, b)
...
def valeurs_entier0(pyromax):  # renvoie (coef0, coef1, coef2, carre0, carre1, carre2, b)
    ...
    while True:
        ...
        c = outils.Arithmetique.valeur_alea(-pyromax, pyromax)
        if b != c and abs(c) != 1:
            break
    return (b, c, a)
...
def valeurs_entier1(pyromax):  # renvoie (coef0, coef1, coef2, carre0, carre1, carre2, b)
    ...
    while True:
        ...
        a0 = (b1 * v0) * v1
        a1 = v1 * b0
        if b0 < b1 and outils.Arithmetique.pgcd(a0, a1) != a1:
            break
