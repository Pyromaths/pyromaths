...
def cercle_trigo(code=0,*args):
    ...
    \\psframe(-2,-2)(2,2)
    \\psaxes[linewidth=0.5pt,Dx=2,Dy=2]{->}(0,0)(-1.25,-1.25)(1.25,1.25)
    ...
    for f in args:
        ...
        if code:
            figure += "\\rput("+str(round(1.1*cos(f),2))+","+str(round(1.1*sin(f),2))+"){"+frac+"}\n"
        else:
            figure += "\\rput("+str(round(1.1*cos(f),2))+","+str(round(1.1*sin(f),2))+"){$M_"+str(idx)+"$}\n"
            idx += 1
...
def findrad(itv=1, neg=0, denom_simple=1):
    ...
    denom_simple : 0 ( quelconque ), 1 ( simple, càd 1, 2, 3, 4, 5, ou 6 )"""
...
def cercle_trigonometrique():
    ...
    mes_deg = [random.randint(0,360) for f in range(5)] # FIX à revoir, trop complexes, traitement des doublons ?
    mes_deg_tex = [u"$%s\\degres$" %(d) for d in mes_deg]
    radq2 = [findrad(2,0,0) for f in range(5)]
    radq3 = [findrad(0,0,0) for f in range(4)] + [findrad(0,1,0)] # dernier négatif
    radq4 = [findrad(0,0,1) for f in range(4)]
    radq5 = [findrad(1,0,1) for f in range(2)] + [findrad(1,1,1)] + [findrad(0,0,1)] # avant-dernier négatif et dernier non principal
    ...
    exo.append(u"\\item Convertir les cinq mesures suivantes en degrés : %s, %s, %s, %s et %s.\\\\\\n" %(tuple(mes_rad_tex[0:5])))
    exo.append(u"\\item Déterminer les mesures principales des angles suivants en radians : %s, %s, %s, %s et %s.\\\\\\n" %(tuple(mes_rad_tex[5:10])))
    exo.append(u"\\item Des angles ont été placés sur le cercle trigonométrique ci-dessous, lire leurs mesures principales en radians.\\\\\\n")
    exo.append(cercle_trigo(0,radq4[0],radq4[1],radq4[2],radq4[3]))
    exo.append(u"\\item Placer les angles suivants sur le cercle trigonométrique : %s, %s, %s et %s.\\\\\\n" %(tuple(mes_rad_tex[14:])))
    ...
    cor.append(u"\\item Convertir les cinq mesures suivantes en degrés : %s, %s, %s, %s et %s.\\\\\\n" %(tuple(mes_rad_tex[0:5])))
    ...
    cor.append(u"\\item Déterminer les mesures principales des angles suivants en radians : %s, %s, %s, %s et %s.\\\\\\n" %(tuple(mes_rad_tex[5:10])))
    ...
    cor.append(u"\\item Des angles ont été placés sur le cercle trigonométrique ci-dessous, lire leurs mesures principales en radians.\\\\\\n")
    ...
    cor.append(u"\\item Placer les angles suivants sur le cercle trigonométrique : %s, %s, %s et %s.\\\\\\n" %(tuple(mes_rad_tex[14:])))
