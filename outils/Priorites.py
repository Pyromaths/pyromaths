...
def split_calc(calcul):
    """Test de gestion et affichage des priorités de calcul, détaillées. PAS FINI."""
    liste_op = []
    liste_result = []
    ...
    while calcul.count('(') > 0:
        current = 'l' + str(i)
        liste_op.append(re.search('\(\w+[\+\-/\*]\w+\)', calcul).group().strip('()'))
    ...
    # Calcul des différentes opérations contenues dans liste_op et enregistrement des résultats dans liste_result
